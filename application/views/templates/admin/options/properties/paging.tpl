...
<b>{$adminLangParams.TITLE_PAGES}: </b>
	{section name=item start=1 loop=$countpage+1 }
	      ...
	      {else}
	        <a href="/admin/options/properties/option_id/{$option_id}/spage/{$spage}/page/{$smarty.section.item.index}" style="font-weight:normal;"><b>{$smarty.section.item.index}</b></a>
	      {/if}
